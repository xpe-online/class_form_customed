...
class Calender:
    ...
    def move(self, wins, hopel, hopew, hopex, hopey, times=1.2):
        ...
        for b in range(len(wins)):
            ...
            while a < 5:
                ...
                win.append(
                    str(int(length_now)) + "x" + str(int(width_now)) + "+" + str(int(x_now)) + "+" + str(int(y_now)))
    ...
    def create(self):
        ...
        self.window.geometry(
            '1x' + str(self.height) + '+' + str(int(self.window.winfo_screenwidth() / 2)) + '+' + str(self.yroot))
        self.window.update()
        ...
        if event.y_root - self.yroot < self.height:
            ...
            if not self.selected:
                if a < 2:  # 选中星期
                    ...
                    self.selectlabs = Label(self.window, text='一 二 三 四 五 六 日', font=('幼圆', 40), fg='white',
                                            bg='black')
                else:
                    self.labels[a]['bg'] = 'grey'
                    self.selectlabs = Label(self.window, text=' '.join(self.selects), font=('幼圆', 40), fg='white',
                                            bg='black')
    ...
    def onclass(self):
        if self.times[0] == [datetime.now().hour, datetime.now().minute]:
            ...
            oncl = Label(self.window, text=self.labels[self.nowclass + 2]['text'], font=('幼圆', 40), fg='yellow',
                         bg='black', wraplength=40)  # 先行创建上方label
            if self.offtk:
    ...
    def offclass(self):
        if self.times[0] == [datetime.now().hour, datetime.now().minute]:
            ...
            self.offtk[0].update()
            self.move([self.window, self.offtk[0]], [self.width, self.x * 2 + self.offtk[1].winfo_reqwidth()],
                      [self.height, self.y * 2 + self.offtk[1].winfo_reqheight()], [int((
                                                                                                    self.window.winfo_screenwidth() - self.width - self.x * 2 -
                                                                                                    self.offtk[
                                                                                                        1].winfo_reqwidth() - 40) / 2),
                                                                                    int((
                                                                                                    self.window.winfo_screenwidth() + self.width -
                                                                                                    self.offtk[
